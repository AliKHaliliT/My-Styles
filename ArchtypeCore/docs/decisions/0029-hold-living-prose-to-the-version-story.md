# 0029. Hold living prose to the version story
...
Record 0028 unified the version story to the one number CI executes, and the
...
The sweep's boundary, config fields only, was never named, and the sibling
trees carried the same rot, Keel's two lines and Helm's two Node lines.
...
remembered list. Keel carries the identical check inside the shared audit
script, and Helm carries the Node form of it against `engines`.
