# 0022. Name a returned value only where its type cannot
...
Ten returns in this repository named an opaque type and now carry a name,
across password verification and hashing, token creation and decoding, the
next free address, the two user counts, and the three shell-facing helpers.
One stutter is gone, since `token : AuthToken` is now bare.
