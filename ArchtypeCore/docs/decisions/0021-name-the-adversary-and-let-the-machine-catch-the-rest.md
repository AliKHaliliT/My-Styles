# 0021. Name the adversary and let the machine catch the rest
...
blind. The full ruleset was run first and reported 23 findings, every one of them a
false positive in context: pytest's own assertions, a container binding on
every interface, the bearer scheme's name read as a credential, and a test
fixture's throwaway secret. Rather than weaken the rule with blanket ignores,
four narrow per-file exclusions carry those four cases and the rule stays at
full strength everywhere else. Each rule was proved against a planted defect before the
