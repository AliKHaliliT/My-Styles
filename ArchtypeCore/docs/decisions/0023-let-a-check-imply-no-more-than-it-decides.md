# 0023. Let a check imply no more than it decides
...
They read any suggestive string as a possible secret, which is why one of them
had already needed a per-file waiver for the bearer scheme's name; that waiver is
gone, since the check no longer blocks anything. The advisory run currently
reports two findings, the bearer constant and a test fixture's throwaway secret,
both correctly surfaced and both dismissed here.
