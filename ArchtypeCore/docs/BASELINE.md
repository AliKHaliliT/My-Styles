...
2. **One-line pitch**, then a short expansion. In a project instantiated from this template, the expansion carries the attribution, one sentence linking the template by name, `Built from [<Template>](<url>), the <kind> template of the My-Styles family`, extended only where the project's provenance needs more, as an arrow's does. It is required because the inherited conventions and decision records are unintelligible without their provenance. The commit the project is aligned to is not written here; it stands on the first line of `UPSTREAM.md`, where a re-alignment starts from it and moves it. The template itself has no upstream, so this applies to derived projects.
3. **The Philosophy: Why Does This Exist?** The problem the project exists to solve. In the template that problem is the drift its architecture defends against, because the architecture is the template's product; in a project built from it, the problem is the one its users have, for whom, and the stance the project takes against the alternatives, while the architecture's own why stays in the map and the records.
4. **The Domain.** What about this domain is harder than it looks, the requirements that live in the corners. The template heads it `The Domain Example: Why ...?` because its domain is a demo chosen to exercise the architecture; a derived project heads it `The Domain: ...` because its domain is real, and it justifies no architecture there.
5. **Core Architectural Pillars.** Numbered, bold-named, and the project's own: adapted to the system, never copied from the template when untrue of it. In a project built from this template the section is short, a few load-bearing decisions each in a sentence with a link into the map, because contributors read the map and users do not.
6. **Project Structure.** A condensed annotated tree; the full map stays in `docs/ARCHITECTURE.md`.
7. **Key Features.** Bulleted, each led by a bold name. A feature states what is, never what changed since a release; a figure may stand once, with the record that measured it, and the comparison belongs in the changelog.
