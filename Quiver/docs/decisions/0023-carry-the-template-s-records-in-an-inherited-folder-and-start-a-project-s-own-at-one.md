# 0023. Carry the template's records in an inherited folder and start a project's own at one
...
In this host, each arrow carries its style's records in its own inherited
folder and numbers its own decisions from 0001. The demo arrow's inherited
folder is held byte-identical to Keel's by the family audit, and its first
own record is its adoption.
