# 0022. Resolve open upstream entries at re-alignment and name no project in the report

Status: Superseded by [0027](0027-keep-upstream-entries-in-one-living-document-until-they-resolve.md)
Date: 2026-09-07
...
In this host, an arrow's upstream report is the arrow's own record, filed in
the `upstream/` folder under the arrow's `docs/`, because the divergence and
the law it answers to are the arrow's and an arrow extracted to stand alone
carries its history with it. The host's report covers the inquiry layer
alone, and one letter may bundle the host's and the arrows' reports when they
are sent, since the family's styles live in one repository.
...
- One report at the host covering every arrow was refused, because the
  ledger of why an arrow differs from its style would then live in a project
  the arrow may leave.
