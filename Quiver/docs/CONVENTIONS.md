...
**Records** describe one past event. A record is written once, dated, and never edited again, apart from its Status line and the repair of a link target that no longer resolves, which moves where a citation points and changes nothing a reader sees. When reality moves past a record, a new record supersedes it. This project keeps three kinds of record: decisions, which hold choices, claims, which hold assertions about the world and the evidence behind them, and review passes, which hold what was read, within what boundary, and what it changed. Any other dated document under `docs/`, a briefing or a progress report, is a record too for the purpose of immutability, though only decisions, claims, and review passes have a shape the audit holds. In a project built from this template, the template's own decision records are carried whole in an `inherited/` folder under `docs/`, byte-identical to the template's at the alignment pin and recopied as one folder at every re-alignment, so `docs/decisions/` holds the project's own decisions and nothing else, numbered from 0001 in every project alike; a record cites an inherited one by a relative link into the `inherited/` folder. A re-alignment is itself one of the project's decisions and gets one record, which cites every record the folder gained and says what each did to the tree, so the records stay the changelog for the child's law as well as the template's. The docs audit fails a record in `docs/decisions/` whose body, beyond its heading and its status line, is an inherited record's, because a template record kept under the project's number is the one copy the split forbids, and the remedy is deletion, since the inherited folder carries the record and history keeps the copy.
...
| `docs/INVARIANTS.md` | Living | The product's stated invariants, one row per claim, each bound to the holder that refuses a violation and graded by what its green is worth. |
...
- No dates and no changelog narration (`STATE.md` and `UPSTREAM.md` entries are the exceptions; each carries an absolute date, its last-verified stamp, and an entry older than 90 days is expired until re-verified, an in-flight `Now` entry after 30, since work that has not moved in a month is finished or stalled).
...
- `AGENTS.md`, `docs/ARCHITECTURE.md`, `docs/BIBLIOGRAPHY.md`, and `docs/INVARIANTS.md` grow with the inquiry rather than against a number, saying everything as briefly as it can be said. Every other living document is bounded at 150 lines, the audit fails one that exceeds its bound, and the remedy is fission by subject. `README.md` stands outside both classes, governed by the README schema in BASELINE.md.
- The mechanical half of freshness is checked by `scripts/audit_inquiry.py`: paths named exist, a token the ignore file covers being read as prose, relative links resolve, in living documents and in the project's own records alike, no `STATE.md` or `UPSTREAM.md` entry outlives its horizon or stands unchanged in the queue for two of them, no bounded document exceeds its budget, every row of the invariants ledger has its three cells, a holder in the tree that contains its needle, and a rung from the list, every document under `docs/` is registered or is a record in a registered record folder, numbered in `decisions/`, `inherited/`, and `claims/` and dated elsewhere, every root entry has a room in the map or the baseline, a record changes only on its Status line or at a link target that comes to resolve, two claims quoting one figure at one pin agree, each manifest names exactly the current claims on its arrow and verifies only those at commits no older than their pins, a recorded claim's preserved artefacts exist, every review pass carries its shape, its boundary's completeness line, a line per stage, and its prior pass, names and schemas hold. Records are exempt from every rule in this section but the one on links; they describe the past, which does not rot, while a path only points at the present.
...
## Rules for the invariants ledger

`docs/INVARIANTS.md` lists what must stay true about the product, one row per claim, in a table whose header row is `| Claim | Held by | Rung |`. The claim is one plain sentence a reader can test. The holder is what refuses a violation, written as the tracked path in backticks, followed where the file holds more than one thing by a quoted needle the file must contain, a test's name, a contract's name, a rule's message; or the holder is the word review, when nothing decides the claim. The rung says what a green result is worth and is one of five words: impossible, where a type, an import contract, or a check over the whole tree refuses the violation before anything ships; generated cases, where a property test fails on a counterexample drawn from generated input; listed cases, where an example test fails on the cases it lists; advised, where a check names the violation on every run and a reader decides; and review, where nothing decides it. No rung claims proof, because a sampled property is not a proof and a passing test is not a theorem, and a holder of review pairs only with the rung review.

The docs audit decides the decidable half, that every row has its three cells, that every holder path is a tracked file containing its needle, that every rung is one of the five words, and that review pairs with review; it prints every review row as an advisory on every run, so a claim nothing decides cannot hide behind a green. Whether a claim is true, whether its holder holds it, and whether every claim the README and the map make has a row are review's. The template's rows are the exemplar and describe its demo. A project writes its own rows at adoption, cut from them, and the ledger is the project's own document from then on, re-adapted at re-alignment like `STATE.md` and never recopied. The shapes a claim takes recur across products, a total conserved, a value kept within bounds, an order preserved, a round trip without loss, an action refused to a role, a failure reverted whole, and the family's treasury lists them in its study of invariant binding.

## Rules for decision records

Write a decision record when a choice shapes future work and its reasoning would otherwise be lost. Records live in `docs/decisions/`, named `NNNN-short-kebab-title.md`, and follow the family template: Status and Date lines, then Context, an optional Evidence section where the decision rests on something measured or run, Options considered where they existed, Decision, and Consequences. An accepted record is immutable; when a decision changes, a new record supersedes it, and flipping the old `Status:` line is the only edit it may receive. A defect found in a record after it merged stays, a wording, a figure or a mark of punctuation alike. A later record supersedes it where the defect changes what was decided, and nothing supersedes a wording, because a record is evidence of its day and never a precedent for new writing; the law a new record is held to is this rulebook's, not an older record's. A dead end that cost real effort or could plausibly be retried gets its record when the evidence arrives, naming what killed it and what would reopen it. The inherited folder keeps the template's numbers untouched and a project's own records start at 0001, so a number is unique within its folder and the two sequences never meet.
...
A claim record is immutable and the `Status:` line is its only legal edit. The trigger for writing or flipping one is evidence arriving or a claim moving, never work completing. A refuted claim names the evidence that killed it, the condition that would reopen it, and the pin that held the attempt. `Stale` means an arrow moved past a claim's pin, or past its latest verification, in the paths that can change a result, its code, its tests, or its project file, so the claim is no longer backed rather than wrong. A person answers the audit's advisory in one of three ways. When the reproduction prints the same figures, the arrow's manifest records the verification as `NNNN at <commit-hash>` and the claim stands with the pin that produced its figures; when the figures, the evidence, or the belief behind the claim change, a new claim supersedes the old, which is the only ground for supersession; otherwise the claim is flipped `Stale`. A recorded observation is never re-verified and never stale. Any completeness assertion inside a claim names the enumerable boundary it exhausted or presents itself as judgment.

## Rules for review records

A review record is the account of one pass over the literature: what slice of the question it served, what it searched and within what bounds, how it read, what it found, and what it changed. Records live in the `reviews/` folder under `docs/`, named `YYYY-MM-DD-short-kebab-title.md`, and follow this template: a `Date:` line, then the sections **Slice**, **Boundary**, **Method**, **Stages**, **Found**, **Changed**, and **Left out**, in that order.
...
A qualifier that bounds a claim is not filler and survives every cut. "No known remaining risks" promises less than "no remaining risks", and the word that made the difference was the claim's boundary, so a pass that removes padding removes what surrounds a claim and never what limits one. The test is whether the sentence promises more without the word; a hedge dropped leaves the promise the same and a boundary dropped enlarges it.
...
Before shipping prose, the writer checks its own output against this section: colons only for lists, quotes, and labels; at most one flourish per paragraph and two em dashes per file; every generic sentence bound to the subject or cut; every emphasis argued rather than labeled; a report to a person opened with its plain account; the ending concrete. Beyond the counts named above, and the splice advisory that names a colon opening a lowercase clause in a record before it lands and decides nothing, no tool judges any of this, so the check is the writer's own, and review reads behind it.
