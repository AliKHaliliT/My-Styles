...
- The mechanical half of freshness is checked by `scripts/audit_inquiry.py`: paths named exist, relative links resolve, no `STATE.md` or `UPSTREAM.md` entry outlives its horizon or stands unchanged in the queue for two of them, no bounded document exceeds its budget, every document under `docs/` is registered or is a record in a registered record folder, numbered in `decisions/`, `inherited/`, and `claims/` and dated elsewhere, every root entry has a room in the map or the baseline, a record changes only on its Status line, two claims quoting one figure at one pin agree, each manifest names exactly the current claims on its arrow and verifies only those at commits no older than their pins, a recorded claim's preserved artefacts exist, every review pass carries its shape, its boundary's completeness line, a line per stage, and its prior pass, names and schemas hold. Records are exempt from every rule in this section; they describe the past, which does not rot.
