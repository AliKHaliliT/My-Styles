"""The two tie-breaking strategies whose accumulated drift the inquiry compares."""
...
def round_half_up(amount: Decimal) -> Decimal:
    """Round an amount to whole cents, breaking ties away from zero.
...
def round_half_even(amount: Decimal) -> Decimal:
    """Round an amount to whole cents, breaking ties toward the even cent.
...
def is_tie(amount: Decimal) -> bool:
    """Whether an amount sits exactly halfway between two cents.
    ...
    -------
    bool
        holds_tie: True when the amount's remainder past whole cents is
        exactly half a cent, which is the only case where the two
        strategies disagree.
