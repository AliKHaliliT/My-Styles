...
- Work on an arrow: use that arrow's own commands, stated in its README; for the demo arrow, `cd arrows/coinwise` then `pip install -e .` with `pip install --group dev`, and its gate commands are `pytest`, `ruff check . && lint-imports`, and `mypy src tests`

The audit reports at two levels. A failure is a verdict, it stops the command, and it means a rule the tool fully decides has been broken. A warning is advice, it leaves the exit status clean, and it comes from a check that cannot decide its own question and so is not allowed to gate. Advice is not noise and not optional reading. Every warning is looked at and then either fixed or dismissed in writing, in the change that produced it, and a warning is never silenced with a suppression to make a run look clean. The advisory check here is the stale-pin scan, which flags every claim whose arrow has moved past its pin; only a person can tell whether the movement touched what the claim measured.
