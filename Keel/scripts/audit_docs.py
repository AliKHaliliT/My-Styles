...
NOW_CAP = 5
# Bounded documents fail past this; AGENTS.md, docs/ARCHITECTURE.md, README.md, the rulebook, and the
# invariants ledger grow with the system instead, a manual, a map, the law it is held to, and its claims.
BUDGET_LINES = 150
FREE_GROWING = {"AGENTS.md", "docs/ARCHITECTURE.md", "README.md", "docs/CONVENTIONS.md", "docs/INVARIANTS.md"}
...
CODESPELL_SKIP = ".git,node_modules,.hypothesis,__pycache__,dist,package-lock.json,*.svg,*.png,*.ico,*.woff,*.woff2,*.map,decisions,claims,reviews,inherited,mockServiceWorker.js,audit_docs.py,audit-docs.mjs,audit_inquiry.py"
...
def check_invariants(problems: list[str], advice: list[str]) -> None:
...
def check_top_level_docs(problems: list[str], docs: Path, agents: str) -> None:
    ...
    for f in sorted(docs.glob("*.md")):
        ...
        if f.name not in ("ARCHITECTURE.md", "CONVENTIONS.md", "BASELINE.md", "UPSTREAM.md", "INVARIANTS.md"):
...
def prove_invariant_plants() -> int:
    ...
    failures = 0
    try:
        problems, advice, _ = run()
        for offset, (row, needle) in enumerate(INVARIANT_PLANTS):
            where = f"docs/INVARIANTS.md:{separator + 2 + offset}:"
            if needle is not None:
                failures += expect(problems, needle, f"invariant plant {row!r}")
            elif any(p.startswith(where) for p in problems):
                failures += wrong(f"the legal invariant row {row!r} was reported")
        failures += expect(advice, "held by review alone", "the review row")
    finally:
...
def prove_splice_advice() -> int:
    """Two spliced clauses in a record main does not hold are advised, one of them across a wrap, the label and the list intro are not, and the record leaves."""
    decisions = ROOT / "docs/decisions"
    if not decisions.is_dir():
    ...
    try:
        ...
        if len(found) == 2 and f"{record.name}:8:" in found[0] and f"{record.name}:13:" in found[1]:
            return 0
        return wrong(f"the planted splices were advised {len(found)} time(s) instead of once each at lines 8 and 13, {found}")
    finally:
        record.unlink()
...
def prove_ignored_term() -> int:
    ...
    if shutil.which("codespell") is None or not readme.exists():
        print("ignored term plant skipped: codespell is not on PATH or no README.md in this tree")
        return 0
    original = readme.read_bytes()
    ...
    try:
        ...
        advise_spelling(advice)
        if any("reciever ==> receiver" in a for a in advice):  # codespell:ignore reciever
            return wrong(f"a misspelling named in {IGNORE_FILE} was still advised")
        return 0
    finally:
        readme.write_bytes(original)
        if had is None:
