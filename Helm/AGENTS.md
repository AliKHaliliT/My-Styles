...
- One working tree and one branch per session, never two sessions in one tree. A change lands by merging main into the branch, running the gate on the merged tree, pushing the branch, waiting for its run to pass, and fast-forwarding main, so main never carries a tree the gate has not seen whole; integration is a merge and not a rebase where records pin commits, because a rebase rewrites the commits they name.
